...
%     end
    ...
    if frame == frame_color(1).black(end)+1%if frame==1 %looking at first frame for roi selection
        ...
        if casenum==0 %if ROI is being selected for the first time 
            ...
            resized_image(sub2ind(size(resized_image),round(size(resized_image,1)/2),round(size(resized_image,2)))) = -1;%set centroid to -1
...
%             hold on
        elseif casenum == 1 %load data from previous ROI drawing
...
%                     plot(xcircle, ycircle);
                ...
                %store full ROI data as well for masking or other analysis
...
%% nested function to get points of a circle
