function [roi_pixels,Sij,roi_num,first_frame] = roi_pix(rawvideo,frame,baseroi_num,analyzeroi_num,seedpixel_roinum,exclude_roi,bin_num,imgname,stopframe,casenum,data,frame_color)

%% ROI selection from user-defined shapes
...
%     end
    ...
    if frame == frame_color(1).black(end)+1%if frame==1 %looking at first frame for roi selection
        if strcmp(frame_color(1).userinput,'b') == 1
            first_frame=single_framegraylarge;%store a copy of first frame
            %if first frame is blue (where rois are desired) store it
        else %first frame is green (occasionally happens, need blue for roi selection)
            r2 = VideoReader(strcat(rawvideo.Path,'\',rawvideo.Name)); %skip to sub-function at bottom of code
            %reread video file and find the first frame after the green
            %frame because ROI selection should be with most illuminated
            %frame, which happens to be the blue frame
            [first_frame] = firstblueframe(r2,frame_color);
        end
...
%                     plot(xcircle, ycircle);
                ...
                %Sij(1).roi = zeros(size(single_framegray,1),size(single_framegray,2));
    %                 if i==baseroi_num+1 %assumes first non-background roi is wholebrain
    %                     %assume analysis rois will be contained within a larger
    %                     %whole brain roi
    %                     Sij(i).roi(sub2ind(size(single_framegray),roi_pixels(i).y_pixels,roi_pixels(i).x_pixels))=single_framegray(sub2ind(size(single_framegray),roi_pixels(i).y_pixels,roi_pixels(i).x_pixels));
    % 
    %                     grayscale_frames(1).allpixel_intensity(:,:,count)=Sij(i).roi;
    %                     %imshow(uint8(Sij))
    %                     %works to set values of new matrix to old ones for summation
    %                 end
...
%                         plot(roi_pixels(k).fullxpixel,roi_pixels(k).fullypixel,color)
        ...
        elseif casenum == 1 %repeat the above if statement, but now instead of drawing rois
            %grab them from a .mat file (use same rois across the same
            %mouse for different videos) or for sensory stimulus analysis
            %where ROIs are only drawn once
            roi_pixels=data.roi_pixels;
            ...
            Sij(1).roigreen = double(single_framegray);
            Sij(roi_num+1).roigreen = double(single_framegraylarge);
            roi_num=baseroi_num+analyzeroi_num;%first roi is always whole frame
            
            for i=2:roi_num %iterate for desired roi #s
                ...
                roi_pixels(i).x_border=round(roi_pixels(i).x_borderfull*bin_num);%xpixel border data for polygon
                roi_pixels(i).y_border=round(roi_pixels(i).y_borderfull*bin_num);
                roi_pixels(i).X=round(roi_pixels(i).Xfull*bin_num);
                roi_pixels(i).Y=round(roi_pixels(i).Yfull*bin_num);
                roi_pixels(i).x_pixels=xpixels;%x pixels between image border
                roi_pixels(i).y_pixels=ypixels;
                ...
                plot(roi_pixels(i).x_borderfull,roi_pixels(i).y_borderfull,'g');
%                 hold on
                geom = roi_pixels(i).geom;
                text(round(geom(2)),round(geom(3)),num2str(i))%add roi# label in the middle of the polygon
                %saveas(gcf,'tst.jpg')
                ...
                if i==roi_num
                    ...
                    img=figure;%save original figure with rois overlayed on it
                    imshow(first_frame)
                    saveas(img,imgname)
                    close(img)
                    ...
                    figure(2); imshow(imresize(data.first_frame,bin_num,'bilinear'));hold on;
                    for k=baseroi_num+1:roi_num
                        if k==baseroi_num+1
                            color='.b';
                        else
