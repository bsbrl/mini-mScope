...
%% colormap gen - new function
%scaled_vec=rescale(currentframe(1:end)',a-del,b+del);%rescale to min and max f/fo across all frames
%[~,ind]=sort(currentframe);%numerically sort all values as they may be out of order, keep indices of where sorted numbers belong
%currentframe(1) = a-del; %rounding issues later on, set hard values for first and end limits
%currentframe(end) = b+del;
